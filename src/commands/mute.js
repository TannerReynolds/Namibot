...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("duration")
        .setDescription("The amount of time to mute this user for")
        .setRequired(true),
    )
    .addStringOption((option) =>
      option
  ...
  async execute(interaction) {
    ...
    if (targetMember) {
      await targetMember
        .send(
          `You have been muted in ${interaction.guild.name} for \`${reason}\`. The length of your mute is ${durationString}.`,
        )
        .catch(() => {});
    }
