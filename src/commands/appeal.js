...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("reason")
        .setDescription("The reason why you should be unbanned")
        .setMaxLength(1_900)
        .setRequired(true),
    ),
    ...
    try {
      ...
      if (existingMail) {
        return sendReply(
          interaction,
          "error",
          "You already have an active mod mail. Please wait for a response before creating another.",
        );
      }
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId(`unbanApprove_${interaction.user.id}`)
          .setLabel("Approve Appeal")
          .setStyle(ButtonStyle.Primary),
        new ButtonBuilder()
          .setCustomId(`unbanDeny_${interaction.user.id}`)
