  ...
  async execute(interaction) {
    ...
    if (interaction.member.roles.highest.position <= selectedRole.position) {
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You dont have high enough permissions to remove this role`,
      );
    }
    if (selectedRole.id === guilds[interaction.guild.id].staffRoleID) {
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove the staff role`,
      );
    }
    if (selectedRole.permissions.has(PermissionFlagsBits.Administrator))
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove roles with the Administrator permission`,
      );
    if (selectedRole.permissions.has(PermissionFlagsBits.BanMembers))
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove roles with the Ban Members permission`,
      );
    if (selectedRole.permissions.has(PermissionFlagsBits.KickMembers))
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove roles with the Kick Members permission`,
      );
    if (selectedRole.permissions.has(PermissionFlagsBits.ManageChannels))
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove roles with the Manage Channels permission`,
      );
    if (selectedRole.permissions.has(PermissionFlagsBits.ManageGuild))
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove roles with the Manage Guild permission`,
      );
    if (selectedRole.permissions.has(PermissionFlagsBits.ManageMessages))
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove roles with the Manage Messages permission`,
      );
    if (selectedRole.permissions.has(PermissionFlagsBits.ManageEvents))
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove roles with the Manage Events permission`,
      );
    if (selectedRole.permissions.has(PermissionFlagsBits.ManageRoles))
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove roles with the Manage Roles permission`,
      );
    if (selectedRole.permissions.has(PermissionFlagsBits.ManageNicknames))
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove roles with the Manage Nicknames permission`,
      );
    if (selectedRole.permissions.has(PermissionFlagsBits.MentionEveryone))
      return sendReply(
        interaction,
        "error",
        `${emojis.error}  You cannot remove roles with the Mention Everyone permission`,
      );
    if (selectedRole.permissions.has(PermissionFlagsBits.ManageNicknames))
      return sendReply(
        interaction,
    ...
    targetMember.roles
      ...
      .then(() => {
        let roleEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `${emojis.success}  Successfully took away <@${target}>'s ${selectedRole.name} role`,
          )
          .setTimestamp()
