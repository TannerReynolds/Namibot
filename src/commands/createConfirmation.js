...
module.exports = {
	...
	async execute(interaction) {
		...
		try {
			...
			return sendReply(interaction, 'error', `${emojis.error}  Error sending confirmation message: ${e}`);
		}
		...
		await prisma.confirmation
			...
			.then(() => {
				return sendReply(interaction, 'success', `${emojis.success}  Confirmation message created!`);
			})
			.catch(e => {
				log.error(`Confirmation could not be created: ${e}`);
				return sendReply(interaction, 'error', `${emojis.error}  Confirmation could not be created: ${e}`);
			});
		log.debug('end');
	},
};
