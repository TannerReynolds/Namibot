...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("action")
        .setDescription("Adding or removing a nitro role?")
        .addChoices(
          { name: "Add", value: "add" },
    ...
    .addStringOption((option) =>
      option
        .setName("role-id")
        .setDescription("ID of the role you'd like to add")
        .setRequired(true)
    )
    .addStringOption((option) =>
      option.setName("emoji").setDescription("Emoji to use for the role")
    ),
  async execute(interaction) {
    ...
    if (!validRole) {
      return sendReply(
        interaction,
        "error",
        "Invalid role input. Must be a valid snowflake representing the role."
      );
    }
