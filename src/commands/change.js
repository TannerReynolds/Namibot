...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption(option => option.setName('username').setDescription('new username'))
		.addStringOption(option => option.setName('avatar-url').setDescription('URL to new avatar')),
	async execute(interaction) {
		...
		if (!uName && !avURL) {
			return sendReply(interaction, 'error', `${emojis.error}  Nothing to change`);
		} else {
			let changeEmbed = new EmbedBuilder()
				.setTitle(`Bot Account Updated`)
				.setColor(colors.success)
				.setDescription(`${emojis.success}  Successfully updated the bot account`)
				.setTimestamp()
