...
module.exports = {
	...
	async execute(interaction) {
		...
		if (!interaction.options.getString('tag-id')) {
			return sendReply(interaction, 'error', 'No tag ID provided!');
		}
		let tagID = interaction.options.getString('tag-id');

		if (isNaN(tagID)) return sendReply(interaction, 'error', 'Please enter the tag ID, the input entered is not a number');
		...
		await prisma.tag
			...
			.catch(e => {
				sendReply(interaction, 'error', `Could not delete tag...\n${e}`);
			});
	},
};
