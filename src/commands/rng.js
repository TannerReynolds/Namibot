...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("reason")
        .setDescription("The reason for subjecting this user to fate")
        .setRequired(true),
    ),
  async execute(interaction) {
    ...
    if (!canDoAction) {
      return sendReply(
        interaction,
        "error",
        "You or the bot does not have permissions to complete this action",
      );
    }
    ...
    interaction.channel
      .send(
        "Thank you for starting a game of RNG! Lets find out what this contestant's fate will be! Lets start by getting a random number between 1 and 1000...",
      )
      .then((m) => {
        interaction.channel.sendTyping();
        setTimeout(() => {
          ...
          m.reply(
            `Ok, I have chosen the number ${chosenTime}! Now lets decide what unit this number should be in. Seconds? Minutes? Hours? let me think for a moment...`,
          ).then((m2) => {
            interaction.channel.sendTyping();
            ...
            setTimeout(() => {
              interaction.channel.sendTyping();
              m2.reply(
                `Alright, I've landed on ${unit}! Now all we have to do is figure out what exactly we're gonna do for ${chosenTime} ${unit}. Will it be a mute or a ban?`,
              ).then((m3) => {
                const pTypes = ["mute", "ban"];
                const randomIndexPType = Math.floor(
                  Math.random() * pTypes.length,
                );
                const punishmentType = pTypes[randomIndexPType];
                interaction.channel.sendTyping();
                setTimeout(() => {
                  m3.reply(
                    `Alright, I've got it! This contestant will get a ${punishmentType}, for ${chosenTime} ${unit}! Thanks for playing RNG!!!`,
                  ).then((m4) => {
                    if (punishmentType === "ban") {
                      let banEmbed = new EmbedBuilder()
                        ...
                        .setDescription(
                          `Banned <@${target}> for ${chosenTime} ${unit}. Reason: ${reason}`,
                        )
                        .setTimestamp()
                      ...
                      let muteEmbed = new EmbedBuilder()
                        ...
                        .setColor(colors.main)
                        .setDescription(
                          `Muted <@${target}> ${chosenTime} ${unit}. Reason: ${reason}`,
                        )
                        .setTimestamp()
                        .setAuthor({ name: name, iconURL: aviURL });
