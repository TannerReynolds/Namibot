...
//const { regexMatch } = require('../utils/processRegex');
    ...
    for (let i = 0; i < nitroColors.length; i++) {
      ...
      dropdown.addOptions(
        new StringSelectMenuOptionBuilder()
          .setEmoji(extractSnowflake(nitroColors[i].emoji))
          .setLabel(nitroColors[i].name)
          .setValue(nitroColors[i].roleID),
      );
    }
...
function extractSnowflake(str) {
  const regex = /(?<=:)(\d+)(?=>)/;
  //const match = regexMatch(regex, str);
