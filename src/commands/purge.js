...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption(option =>
			option.setName('includes')
				.setDescription('Filter messages that include specific content'))
        .addBooleanOption(option =>
            option.setName('bots')
                .setDescription('Filter messages sent by bots')),
	async execute(interaction) {
        ...
        await channel.bulkDelete(filteredMessages, true)
            .then(messages => {
				sendReply('main', `Successfully deleted ${messages.size} messages.`)
		    })
            .catch(error => {
                log.error(error);
                sendReply('error', 'There was an error trying to purge messages in this channel!');
            });
