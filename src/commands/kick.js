...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (targetMember) {
      await targetMember
        .send(
          `You have been kicked from ${interaction.guild.name} for \`${reason}\`. Feel free to rejoin using this link:\n${guilds[interaction.guild.id].invite}`,
        )
        .catch(() => {
          // do nothing
        });
    ...
    targetMember
      .kick(reason) // This needs to include moderator information
      .then(() => {
        let kickEmbed = new EmbedBuilder()
          .setTitle(`User Kicked`)
          .setColor(colors.main)
          .setDescription(
            `${emojis.success}  Kicked <@${target}>. Reason: ${reason}`,
          )
          .setTimestamp()
      ...
      .catch((e) => {
        return sendReply(
          interaction,
          "error",
          `${emojis.error}  Error when attemping to kick member:\n${e}`,
        );
      });
