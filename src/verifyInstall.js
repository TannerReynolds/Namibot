...
const isDiscordEmoji = (emoji) => /^<:[^:]+:\d+>$/.test(emoji);
const isUnicodeEmoji = (emoji) => emoji.length === 2 && emoji.charCodeAt(0) === 55357;
...
async function verifyInstall() {
    ...
    try {
        ...
        failures.push("Could not find the config.json file. Please rename example.config.json to config.json and then ensure all information is correctly entered.")
    }
    ...
    if(config) {
        ...
        for (const emoji in config.emojis) {
            const value = config.emojis[emoji];
            if (!isUnicodeEmoji(value) && !isDiscordEmoji(value)) {
                failures.push("Please ensure all values here are proper emojis. They can be eitrher unicode characters or discord emoji inputs")
            }
        }

        if(typeof config.server.enabled !== 'boolean') {
            failures.push("Please ensure that server.enabled is a boolean. A boolean is `true` or `false` without the backticks. They are not to be confused with strings and should not be encased in \". Example: enabled: true,")
        }
        ...
        for (const guildId in config.guilds) {
            ...
            if (guild.features.antiAds.enabled && (!Array.isArray(guild.features.antiAds.allowedInvites) || !guild.features.antiAds.allowedInvites.every(isUrl))) {
                failures.push("Your anti Ads allowedInvites array is either not of type array<String>, or it is not filled with full URLs. Please ensure each invite is the full URL and not just the domain and path. Example of correct allowedInvites array: \"allowedInvites\": [\"https://discord.gg/cars\", \"https://discord.gg/learnjapanese\"]")
            }
        }
    }
