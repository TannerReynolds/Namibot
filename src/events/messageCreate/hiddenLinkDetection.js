async function checkForInlineURLs(client, content, message, getModChannels) {
	...
	if (content.match(inLineRegex)) {
		message.reply(`Inline/hidden URL detected. URLs found in message: ${content.match(urlRegex).join(', ')}`).then(r => {
			getModChannels(client, message.guild.id).main.send(`<@${message.author.id}>(${message.author.id}) sent inline/hidden URLs: ${content.match(urlRegex).join(', ')}\n[${message.url}]\n`);
		});
	}
}
