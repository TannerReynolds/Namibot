  ...
  bannedsearch: for (let word of words) {
    ...
    for (let bannedWord of bannedWords) {
      if (word.toLowerCase().includes(bannedWord.toLowerCase())) {
        ...
        message
          .reply(
            "Banned word detected, please read and follow our rules regarding allowed language."
          )
          .then((r) => {
            modChannels[message.guild.id].send(
              `<@${message.author.id}>(${message.author.id}) sent banned word: \`${word}\``
            );
            message.delete();
            setTimeout(() => {
              r.delete();
