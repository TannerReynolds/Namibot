...
async function unbanButtonDeny(interaction, args) {
	...
	const finishedRow = new ActionRowBuilder().addComponents(
		new ButtonBuilder().setCustomId(`disabledApprove`).setLabel('Approve Appeal').setStyle(ButtonStyle.Secondary).setDisabled(true),
		new ButtonBuilder().setCustomId(`disabledDeny`).setLabel('Deny Appeal').setStyle(ButtonStyle.Secondary).setDisabled(true)
	);
	...
	if (targetUser) {
		try {
			await targetUser.send(`Your ban appeal in ${interaction.guild.name} has been denied.`);
		} catch (e) {
			// do nothing
		}
	}
