...
async function reportSubmission(interaction, args) {
  ...
  if (!message) {
    return sendReply(
      interaction,
      "error",
      "Message does not exist or could not retrieve message",
    );
  }

  let mailChannel = await guild.channels.cache.get(
    guilds[interaction.guild.id].mailChannelID,
  );
  ...
  const rowPunishments = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`ban_${message.author.id}`)
      .setLabel("Ban Reported User")
      .setStyle(ButtonStyle.Danger),
    new ButtonBuilder()
      .setCustomId(`kick_${message.author.id}`)
      .setLabel("Kick Reported User")
      .setStyle(ButtonStyle.Danger),
    new ButtonBuilder()
      .setCustomId(`mute_${message.author.id}`)
      .setLabel("Mute Reported User")
      .setStyle(ButtonStyle.Danger),
    new ButtonBuilder()
      .setCustomId(`warn_${message.author.id}`)
      .setLabel("Warn Reported User")
      .setStyle(ButtonStyle.Danger),
  );

  let mailEmbed = new EmbedBuilder()
    .setTitle(
      `Message Report From ${interaction.user.username} (${interaction.user.id})`,
    )
    ...
    .addFields({
      name: "Message Connection Opened With Reporter",
      value: `${connectionMade}`,
    })
    .setAuthor({ name: interaction.user.username, iconURL: aviURL });

  if (typedComments) {
    mailEmbed.addFields({
      name: "Additional Comments From Reporter",
      value: typedComments,
    });
  }
  ...
  mailChannel.threads
    .create({
      ...
      message: {
        embeds: [mailEmbed],
        content: `Creator: <@${interaction.user.id}> | Member Reported: <@${message.author.id}>`,
        components: [row, rowPunishments],
      },
    })
    .then((forumPost) => {
