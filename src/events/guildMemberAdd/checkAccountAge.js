  ...
  if (age > weekAgo) {
      ...
      .then(() => {
        let logEmbed = new EmbedBuilder()
          ...
          .addFields(
            ...
            {
              name: "Reason",
              value: `Account is newer than ${daysReq} days | Creation date: ${age}`,
            },
            { name: "Moderator", value: "System" },
          )
          .setTimestamp();

        let logChannel = member.client.guilds.cache
          .get(member.guild.id)
          .channels.cache.get(c.guilds[member.guild.id].mainLogChannelID);
        logChannel.send({
          embeds: [logEmbed],
    ...
    if (count > 3) {
      ...
      member.guild.bans
        ...
        .then(() => {
          let logEmbed = new EmbedBuilder()
            ...
            .addFields(
              ...
              { name: "Reason", value: `Suspected bot account` },
        ...
        .catch((e) => {
          log.error(
            `Error on banning user automatically (newer than ${daysReq} days old):\n${e}`,
          );
        });
    }
  }
