...
const { initGuildMemberCache, syncMemberCache } = require(
  `${utils}guildMemberCacheSynch`,
);
...
const { antiChannelNuke, channelDeletor } = require(
  `${cDelete}antiChannelNuke`,
);
...
client.once(Events.ClientReady, async (c) => {
  ...
  log.success(
    `Watching over ${client.guilds.cache.size} guilds and ${c.users.cache.size} users`,
  );

  if (fs.existsSync("./restart.js")) {
    try {
      log.success("Restart successful!");
      let restartEmbed = new EmbedBuilder()
        .setColor(colors.success)
        .setDescription("Successfully restarted bot!")
        .setTimestamp();
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (ratelimited.has(interaction.user.id)) {
    let cooldownEmbed = new EmbedBuilder()
      .setTitle(`Please wait a few seconds before running another command!`)
  ...
  try {
    if (
      ...
      pingStaffRatelimited.has(interaction.guild.id)
    ) {
      let cooldownEmbed = new EmbedBuilder()
        .setTitle(
          `This command can only be ran once every 15 minutes in each guild`,
        )
        .setColor(colors.main)
        .setTimestamp();
      return interaction.reply({ embeds: [cooldownEmbed] });
  ...
  try {
    if (interaction.commandName === "pingstaff" && interaction.guild.id) {
      ...
      setTimeout(
        () => pingStaffRatelimited.delete(interaction.guild.id),
        900000,
      );
    }
  } catch (e) {
    ...
    await prisma.mail
      ...
      .then(() => {
        channel.client.users.cache
          .get(mail.userID)
          .send(
            `Your mod mail connection in ${channel.guild.name} has been closed by a staff member.`,
          )
          .catch((e) => {
            log.error(e);
          });
...
client.on(Events.GuildMemberAdd, async (member) => {
  if (guilds[member.guild.id].features.checkAccountAge.enabled)
    checkAccountAge(member);
  if (guilds[member.guild.id].features.autoRole.enabled) autoRole(member);
});

client.on(Events.GuildMemberUpdate, async (oldMember, newMember) => {
  if (guilds[newMember.guild.id].features.nitroRoles.enabled)
    checkBoosterStatus(oldMember, newMember);
});
      ...
      await prisma.mail
        ...
        .then(() => {
          newThread.client.users.cache
            .get(mail.userID)
            .send(
              `Your mod mail connection in ${newThread.guild.name} has been closed by a staff member.`,
            )
            .catch((e) => {
              log.error(e);
            });
...
client.on(Events.MessageUpdate, async (oldMessage, message) => {
  ...
  try {
    if (guilds[message.guild.id].features.antiAds.enabled)
    ...
    let isStaffBool = await isStaff(
      message,
      guildMember,
      PermissionFlagsBits.ManageMessages,
    );
    await messageEvents(isStaffBool, message, oldMessage);
    if (guilds[message.guild.id].logs.messageUpdate)
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (guilds[guildID].features.antiSpam && !isStaffBool)
  ...
  try {
    if (
      !guildMemberCache[guildID] ||
      !guildMemberCache[guildID][message.author.id]
    ) {
      guildMemberCache[guildID][message.author.id] = {
        xp: 0,
        level: 1,
    ...
    if (guilds[guildID].features.levels.enabled) {
      ...
      let totalMessages =
        guildMemberCache[guildID][message.author.id].totalMessages || false;
      if (!totalMessages) {
...
async function messageEvents(isStaffBool, message, guildMember, oldMessage) {
  ...
  try {
    ...
    if (guilds[guildID].features.fileTypeChecker)
...
if (server.enabled) {
  ...
  app.listen(server.PORT, "0.0.0.0", () => {
    log.success(
      `Local server listening on port ${server.PORT} at ${server.url}`,
    );
    if (prod) {
      setTimeout(() => {
        axios
          ...
          .catch((error) =>
            log.error("Failed to register with monitor app:", error.message),
          );
      }, 3000);
    }
  });
